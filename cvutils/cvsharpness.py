...
class CVSharpness:
    def __init__(self, use_sobel=False):
        self.kernel_x = np.array([(0, 0, 0), (-1, 0, 1), (0, 0, 0)], np.float)
        self.kernel_y = np.array([(0, -1, 0), (0, 0, 0), (0, 1, 0)], np.float)
        if use_sobel:
    ...
    def calculate_sharpness_video_capture(self,
        ...
        frame_sharpness = np.zeros([frame_count], np.float)
        ...
        def calculate_sharpness_worker(joinable_queue: JoinableQueue):
            joinable_queue
        ...
        #     pool.starmap(calculate_sharpness_worker, args_list)
    ...
    def calculate_sharpness_frame(self, cv_frame,
                                  gray_scale_conversion_code
                                  =cv2.COLOR_BGR2GRAY):
        if cv_frame is None:
            return 0
        height = cv_frame.height
        width = cv_frame.width
        frame_gray = cv2.cvtColor(cv_frame.cv_mat, gray_scale_conversion_code)
